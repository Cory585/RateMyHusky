...
export function loadAskSession(): AskSession | null {
  try {
    ...
    // Other answer-bearing modes (course_list) have no sources and are fine to restore.
    const result = parsed.result as { mode?: unknown; sources?: unknown };
    if (result.mode === 'question' && !Array.isArray(result.sources)) return null;
    return parsed;
  } catch { return null; }
}
