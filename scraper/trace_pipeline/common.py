"""Shared helpers for the TRACE Bluera pipeline: DB connect w/ DNS retry,
serialization-retry writes, name normalization, cookie-file parsing.

Usage:  python scraper/trace_pipeline/common.py --selftest
"""
...
def load_cookies(path):
    if not os.path.exists(path) or not open(path, encoding="utf-8").read().strip():
        sys.exit(f"Cookie file missing/empty: {path}\n"
                 "Open DevTools on northeastern-bc.bluera.com -> Network -> any request ->\n"
...
def selftest():
    ...
    # ── normalize_name (must match backend/precompute.py behavior) ──
    check("normalize collapses ws + lowercases", normalize_name("  Jane   DOE ") == "jane doe")
    check("normalize strips accents", normalize_name("Núñez") == "nunez")
